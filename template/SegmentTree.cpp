...
struct SegmentTree
{
    ...
    vector<int> G;
    vector<Node> tree;
    SegmentTree(vector<int> &g) //[1,n]
    {
        G = g;
        tree.resize(g.size() << 2);
    }
    ...
    void pushDown(int node) // lazy标记下放
    {
        ...
        tree[lc].val += (tree[lc].r - tree[lc].l + 1) * tree[node].lazy;
        tree[rc].val += (tree[rc].r - tree[rc].l + 1) * tree[node].lazy;
...
void solve()
{
    ...
    while (m--)
    {
        ...
        if (op == 1)
        {
            int x, y, k;
            cin >> x >> y >> k;
            ST.update(1, x, y, k); // [x,y]位置加上k
        }
        else
        {
            int x, y;
            cin >> x >> y;
            cout << ST.query(1, x, y) << endl; //[x,y]的和
        }
    }
}
int main()
